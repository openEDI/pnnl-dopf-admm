...
def main() -> None:
    ...
    # Auto-resolve scenario specific output directory if present
    scenario_specific_dir = data_dir / scenario_path.stem
    if scenario_specific_dir.exists() and (scenario_specific_dir / "topology.json").exists():
        data_dir = scenario_specific_dir
    ...
    if not topology_path.exists():
        logger.error(f"Topology file topology.json not found in output directory: {data_dir}")
        sys.exit(1)
    ...
    # Strict validation: Ensure output feather data in data_dir matches topology.json
    if "feeder_v_real" in data and not data["feeder_v_real"].empty:
        recorded_buses = {c.split(".")[0] for c in data["feeder_v_real"].columns if c != "time"}
        topology_buses = {b.split(".")[0] for b in topology.base_voltage_magnitudes.ids}
        if not recorded_buses.intersection(topology_buses):
            rec_sample = list(recorded_buses)[:3]
            top_sample = list(topology_buses)[:3]
            logger.error(
                f"\n[DATA MISMATCH ERROR] Recorded output data in '{data_dir}' does not match "
                f"scenario topology in '{topology_path}'.\n"
                f"Recorded output nodes: {rec_sample}\n"
                f"Topology nodes:        {top_sample}\n"
                f"Ensure that '{data_dir}' contains output data generated by running '{scenario_path.name}'."
            )
            sys.exit(1)
    ...
    if fig_adeq:
        fig_adeq.savefig(output_dir / f"admm_{model}_generation_adequacy.png", dpi=300, bbox_inches="tight")
